...
export default class MoviesDAO {
  ...
  static async getMoviesByCountry(countriesList){
    try {
      const mongoQuery = { countries: { $in: countriesList } }
      const projection = { 'title': 1 }
      return await movies.find(mongoQuery).project(projection).toArray()
    } catch (e) {
      console.error(`You did something wrong in getMoviesByCoutry: ${e}`)
      throw e
    }
  }
}
